...
- [ ] 批处理 > 200fps (batch=8)
- [ ] 显存占用 < 2GB（仅用于AI模型与中间Tensor）
- [ ] 多对多绑定正常工作（流↔任务↔模型）
- [ ] 动态修改实时生效（通过JSON配置与REST API）

**设计约束（对应8+1原则）**

- **模块化**：ONNX推理、模型管理、YOLO检测、批处理、任务管理各自独立命名空间 `mediakit::ai` 子模块，不侵入现有核心代码
- **配置化**：所有任务、模型、阈值、批大小等通过 `ai_tasks.json` / REST API 配置，支持热更新
- **可视化管理**：任务/模型/推理统计通过 REST `/api/ai/statistics` 与日志输出，JSON 结构稳定
- **最小化影响**：默认 `enable=0` 时完全旁路；不改变任何现有流媒体对外 API
- **向后兼容**：AI 模块仅追加功能，不移除或修改现有行为；旧部署不配置 `[ai]` 段仍可正常运行
- **代码注释**：所有 public 接口与关键实现（如批处理、YOLO 后处理）要求有 Doxygen 注释
- **性能**：GPU 只用于 AI 推理；CPU 解码与业务逻辑保持独立线程池，保证端到端延迟与吞吐目标
- **配置集成**：与 `conf/config.ini`、现有 WebApi 保持统一风格，避免新增独立配置系统
- **实时性/抽帧**：严格使用可配置抽帧（`skip_frames`）而非合并帧，单帧实时处理，确保流媒体时序不被打乱
...
- [ ] 调度延迟 < 5ms
- [ ] 规则匹配 < 1ms/规则
- [ ] 告警延迟 < 50ms（从检测结果产生到 WebHook 送达）
- [ ] 吞吐 > 10000结果/秒

**设计约束（对应8+1原则）**

- **模块化**：任务调度器、告警引擎、通知器独立实现，通过清晰接口与 `AITaskManager` / `AIPipeline` 解耦
- **配置化**：告警规则、阈值、冷却时间等全部通过 `alert_rules.json` 与 REST API 管理
- **可视化管理**：规则命中次数、告警速率等可通过统计接口导出，用于看板展示
- **最小化影响**：告警与通知仅消费检测结果，不反向影响流媒体编解码和推流逻辑
- **向后兼容**：未开启 `[alert]` 时不加载规则文件，不创建任何额外线程
- **代码注释**：规则匹配、限流/去重等关键逻辑要求有清晰注释，便于后期扩展更多条件类型
- **性能**：调度/规则匹配设计为 O(n 规则) 或更优，支持高并发检测结果输入
- **配置集成**：沿用现有 `config.ini` 与 WebApi 鉴权/路径风格，不新增独立端口或协议
- **实时性/抽帧**：调度尊重上游 `skip_frames` 策略，不在本阶段引入额外缓冲队列导致延迟堆积
...
- **模块化**：PythonBridge、PythonPluginManager、Python SDK 互相解耦，只通过 C++/Python 明确接口交互
- **配置化**：插件列表、绑定关系、并发度等通过配置文件与 REST API 管理，而不是硬编码
- **可视化管理**：每个插件的调用次数、错误率、平均耗时通过统计接口输出
- **最小化影响**：插件仅消费检测结果与上下文，不直接改动流媒体核心；插件异常时自动隔离，不影响主流程
- **向后兼容**：未启用 `[plugin]` 或未加载插件时代码路径与旧版本一致
- **代码注释**：所有对外暴露给 Python 的接口（`ZLMPlugin`、`PluginContext` 等）要求有完整注释与示例
- **性能**：通过异步执行池与超时控制保证 Python 调用不拖慢 AI 推理与推流线程
- **配置集成**：沿用现有 `config.ini` / WebApi 风格，不新造独立管理端
- **实时性/抽帧**：插件只在检测结果层面工作，不引入额外帧缓冲和排队
...
- **模块化**：REST API、WebSocket 推送、性能调优、测试工具在代码结构上分离，通过清晰接口与 AI 模块交互
- **配置化**：所有新 API 行为受 `config.ini` 中对应开关与限流参数控制，可在运行时调整
- **可视化管理**：性能调优阶段产出的指标（CPU/GPU 利用率、QPS、延迟分布）可通过 API 导出，便于接入监控
- **最小化影响**：新增 API 不改变已有接口语义；未开启 AI/告警/插件时，旧 API 行为保持不变
- **向后兼容**：REST 路径、参数、返回 JSON 保持兼容，新增字段只做追加不修改已有字段含义
- **代码注释**：所有公开 API（特别是 `/api/ai/*`、`/api/alert/*`、`/api/plugin/*`）有清晰注释与示例请求
- **性能**：API 处理线程与媒体处理线程分离；高并发场景下仍满足响应时间目标
- **配置集成**：新配置项统一收敛到现有配置系统与文档中，不增加额外独立配置文件类型
- **实时性/抽帧**：任何监控/统计/推送逻辑不得阻塞媒体与 AI 主路径，只读取状态或异步推送
